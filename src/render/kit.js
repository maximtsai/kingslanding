...
export function createKit(THREE) {
  ...
  const mat = color => {
    if (!materialCache.has(color)) {
      // No flatShading here: MeshLambertMaterial shades per-vertex and ignores
      // the flag, warning once per material if you pass it. The faceted look
      // does not come from it anyway -- box and extrude geometries already carry
      // hard per-face normals, so every facet shades flat regardless.
...
export function createSoftSprites(THREE, scene) {
